...
function App() {
  ...
  const handleGenerateMCQs = async (query, indexName) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/75605149-f19c-434c-b2cc-15ab6991e8e3/query-ollama/?query=${encodeURIComponent(query)}&index_name=${encodeURIComponent(indexName)}`);
      if (!response.ok) {
